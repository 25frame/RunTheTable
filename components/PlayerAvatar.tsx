...
function getInitials(name: string) {
  ...
  if (parts.length === 0) return "RTT";
  if (parts.length === 1) return parts[0].slice(0, 2).toUpperCase();

  return `${parts[0][0]}${parts[parts.length - 1][0]}`.toUpperCase();
}

export function PlayerAvatar({ player }: { player: RTTPlayer }) {
  ...
  return (
    <div className="relative flex h-full w-full items-center justify-center overflow-hidden bg-black">
      <img
        src="/rtt-logo.png"
        alt="RTT NYC"
        className="absolute inset-0 h-full w-full object-contain p-5 opacity-25"
      />
      ...
      <div className="relative flex h-24 w-24 items-center justify-center rounded-full border border-white/15 bg-black/75 shadow-2xl shadow-black/60">
        <span className="text-4xl font-black italic tracking-[-0.08em] text-white">
          {initials}
        </span>
      </div>
    </div>
