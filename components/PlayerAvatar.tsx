...
/** Shows real player photo or premium RTT initials fallback. */
export function PlayerAvatar({ player }: { player: Pick<RTTPlayer, "name" | "photo"> }) {
  ...
  if (hasPhoto) {
    return <img src={player.photo} alt={player.name} className="h-full w-full object-cover grayscale transition group-hover:grayscale-0" />;
  }
  return (
