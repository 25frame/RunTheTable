...
export function PlayerAvatar({ player }: { player: Pick<RTTPlayer, "name" | "photo"> }) {
  ...
  return (
    <div className="relative flex h-full w-full items-center justify-center overflow-hidden bg-black">
      <img src="/rtt-logo.svg" alt="RTT NYC" className="absolute inset-0 h-full w-full object-contain p-5 opacity-25" />
      <div className="absolute inset-0 bg-gradient-to-br from-rtt-red/25 via-black/30 to-black" />
      <div className="relative flex h-20 w-20 items-center justify-center rounded-full border border-white/15 bg-black/75 shadow-2xl shadow-black/60">
        <span className="text-3xl font-black italic tracking-[-0.08em] text-white">{initials}</span>
      </div>
    </div>
  );
}
