/**
 * RTT NYC COMPLETE CODE.GS — TOKEN AUTH + SUPABASE PHOTO URL
 * ==========================================================
 *
 * Full Apps Script backend replacement.
 * Does not use DriveApp for photos. Supabase uploads are handled by the browser.
 * Apps Script stores the resulting Supabase photo URL in Players column I.
 */
...
const AUTH_SECRET = "rtt_secret_VTbya4C8eZPah2YFU5m0QbetNJddBzgjM5SyFmQEBexOXcSA";
const TOKEN_TTL_MS = 1000 * 60 * 60 * 24 * 7;

const DEFAULT_ADMIN_EMAIL = "admin@rttnyc.com";
...
function runSetup() {
  ...
  PropertiesService.getScriptProperties().setProperty("RTT_FORM_URL", RTT_FORM_URL);
  ensureUsersSheet_();
  return { ok: true, message: "RTT setup complete. Users tab verified." };
}
...
function ensureUsersSheet_() {
  ...
  if (sh.getLastRow() === 0) {
    ...
    sh.appendRow(["USR-001", DEFAULT_ADMIN_EMAIL, DEFAULT_ADMIN_PASSWORD, "admin", ""]);
  }
}
...
function login_(payload) {
  ...
  for (let i = 1; i < users.length; i++) {
    const rowEmail = String(users[i][1] || "").toLowerCase().trim();
    const rowPassword = String(users[i][2] || "");
...
function updatePlayerProfile_(payload, user) {
  ...
  if (!sheet) return { ok: false, error: "Missing Players tab." };
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]) === playerId) {
      ...
      return { ok: true, playerId: playerId, message: "Player profile updated." };
    }
  }
...
function syncFormToSystem() {
  ...
  try {
    ...
    if (!formSheet || !playersSheet || !regSheet || !setupSheet) {
      return { ok: true, warning: "Required tab missing; sync skipped." };
    }
    ...
    for (let i = 1; i < formRows.length; i++) {
      ...
      if (found) {
        ...
        const newPlayer = {
          ...
          email: email,
          displayName: norm_(displayName),
          fullName: norm_(fullName)
        };
...
function buildPlayerIndex_(rows) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const player = {
      id: id,
      rowNumber: i + 1,
      displayName: norm_(rows[i][1]),
      fullName: norm_(rows[i][2]),
      email: normalizeEmail_(rows[i][3])
    };
...
function findExistingPlayer_(index, email, displayName, fullName) {
  ...
  const d = norm_(displayName);
  const f = norm_(fullName);
...
function buildRegistrationIndex_(rows) {
  ...
  for (let i = 1; i < rows.length; i++) {
    const eventId = val_(rows[i][0]);
    const playerId = val_(rows[i][1]);
...
function nextPlayerId_(playersData) {
  ...
  for (let i = 1; i < playersData.length; i++) {
    const id = String(playersData[i][0] || "");
...
function recalcStandings() {
  ...
  matchesData.slice(1).forEach(function(row) {
    const playerAId = val_(row[4]);
    const playerBId = val_(row[6]);
    const scoreA = Number(row[8]) || 0;
    const scoreB = Number(row[9]) || 0;
    const winnerId = val_(row[10]);
    ...
    stats[playerAId].matches++;
    stats[playerBId].matches++;
    stats[playerAId].pointsFor += scoreA;
    stats[playerAId].pointsAgainst += scoreB;
    stats[playerBId].pointsFor += scoreB;
    stats[playerBId].pointsAgainst += scoreA;
  ...
  const rows = Object.keys(stats)
    .map(function(id) {
      const s = stats[id];
      s.points = s.wins * winPoints + s.losses * lossPoints;
      s.pointDiff = s.pointsFor - s.pointsAgainst;
      s.winPct = s.matches ? s.wins / s.matches : 0;
      return s;
    })
    .sort(function(a, b) {
      return b.points - a.points || b.wins - a.wins || b.pointDiff - a.pointDiff || a.name.localeCompare(b.name);
    });

  standingsSheet.clearContents();
  standingsSheet.getRange(1, 1, 1, 13).setValues([[
    "Rank", "Player ID", "Display Name", "Skill Level", "Matches", "Wins", "Losses",
    "Points", "For", "Against", "Point Diff", "Win %", "Streak / Notes"
  ]]);

  if (rows.length) {
    standingsSheet.getRange(2, 1, rows.length, 13).setValues(
      rows.map(function(s, i) {
        return [i + 1, s.id, s.name, s.skill, s.matches, s.wins, s.losses, s.points, s.pointsFor, s.pointsAgainst, s.pointDiff, s.winPct, s.streak];
      })
    );
...
function setPayoutFromStandings() {
  const ss = SpreadsheetApp.openById(RTT_SHEET_ID);
  const standings = getStandings_(ss);
  const payouts = ss.getSheetByName("Payouts");
...
function getPlayersForSite_(ss) {
  const standings = getStandings_(ss);
  const playerMap = getPlayerMap_(ss);
  ...
  return values.slice(1)
    ...
    .map(function(r) {
      return {
        ...
        name: val_(r[2]),
        skill: val_(r[3]),
        matches: Number(r[4]) || 0,
        wins: Number(r[5]) || 0,
        losses: Number(r[6]) || 0,
        points: Number(r[7]) || 0,
        pointsFor: Number(r[8]) || 0,
        pointsAgainst: Number(r[9]) || 0,
...
function getPlayerMap_(ss) {
  ...
  values.slice(1).forEach(function(r) {
    ...
    map[id] = {
      displayName: val_(r[1]),
      fullName: val_(r[2]),
      email: val_(r[3]),
      phone: val_(r[4]),
      instagram: val_(r[5]),
      skill: val_(r[6]),
      paymentHandle: val_(r[7]),
      photo: val_(r[8]),
      status: val_(r[9])
    };
  });
  ...
  return values.slice(1)
    ...
    .map(function(r, idx) {
      return {
        ...
        winnerId: val_(r[10]),
        winner: val_(r[11]),
        verified: r[12] === true || String(r[12]).toUpperCase() === "TRUE",
        status: val_(r[13]) || (r[12] === true ? "Final" : "Scheduled"),
        score: String((r[8] || 0) + "-" + (r[9] || 0))
      };
    })
    .reverse();
}
...
function getWeeklyResults_(ss) {
  const payout = getPayout_(ss);

  return [{
    week: payout.week,
    winner: payout.firstPlaceName || "TBD",
    players: payout.paidPlayers,
    collected: payout.totalCollected,
    organizerCut: payout.operationsCut,
    prizePool: payout.prizePool,
    first: payout.firstPlacePayout,
    second: payout.secondPlacePayout,
    third: payout.thirdPlacePayout
  }];
}
...
function getPayout_(ss) {
  ...
  return {
    eventId: eventId,
    week: Number(eventId.replace(/[^0-9]/g, "")) || 1,
    paidPlayers: Number(sh.getRange("B3").getValue()) || 0,
    totalCollected: Number(sh.getRange("B5").getValue()) || 0,
    operationsCut: Number(sh.getRange("B6").getValue()) || 0,
    prizePool: Number(sh.getRange("B7").getValue()) || 0,
    firstPlaceName: val_(sh.getRange("C17").getValue()) || "TBD",
    firstPlacePayout: Number(sh.getRange("B11").getValue()) || 0,
    secondPlacePayout: Number(sh.getRange("B12").getValue()) || 0,
    thirdPlacePayout: Number(sh.getRange("B13").getValue()) || 0
  };
}
