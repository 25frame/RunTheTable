/**
 * RTT NYC PRODUCTION GOOGLE APPS SCRIPT BACKEND
 * ============================================
 *
 * Paste this entire file into Apps Script as Code.gs.
 * Delete old duplicate .gs files that define doGet, doPost, RTT_SHEET_ID, or ADMIN_KEY.
 */
...
const ADMIN_KEY = "rtt_vQWj1twDDrrciUqULZrRlEdRKjJKOQhRpnbs";
...
function doPost(e) {
  ...
  if (request.key !== ADMIN_KEY) return json_({ ok: false, error: "Unauthorized" });
  if (request.action === "saveLiveMatch") return json_(saveLiveMatch_(request.payload || {}));
  if (request.action === "syncForm") return json_(syncFormToSystem());
...
function syncFormToSystem() {
  ...
  try {
    ...
    if (!formSheet) throw new Error("Missing tab: " + FORM_RESPONSE_TAB);
    if (!playersSheet) throw new Error("Missing tab: Players");
    if (!regSheet) throw new Error("Missing tab: Registrations");
    if (!setupSheet) throw new Error("Missing tab: Setup");
...
function recalcStandings() {
  ...
  if (rows.length) {
    standingsSheet.getRange(2, 1, rows.length, 13).setValues(rows.map(function(s, i) {
...
function getPlayersForSite_(ss) {
  ...
  return standings.map(function(s) {
    ...
    return { rank: s.rank, id: s.id, name: s.name, skill: s.skill, wins: s.wins, losses: s.losses, points: s.points, gameDiff: 0, pointDiff: s.pointDiff, streak: s.streak, handle: p.instagram || "", photo: p.photo || "" };
  });
}
...
function getStandings_(ss) {
  ...
  return values.slice(1).filter(function(r) { return r[1] && r[2]; }).map(function(r) {
    return { rank: Number(r[0]) || 0, id: val_(r[1]), name: val_(r[2]), skill: val_(r[3]), matches: Number(r[4]) || 0, wins: Number(r[5]) || 0, losses: Number(r[6]) || 0, points: Number(r[7]) || 0, pointsFor: Number(r[8]) || 0, pointsAgainst: Number(r[9]) || 0, pointDiff: Number(r[10]) || 0, winPct: r[11] || 0, streak: val_(r[12]) };
  });
}
...
function getPlayerMap_(ss) {
  ...
  values.slice(1).forEach(function(r) {
    const id = val_(r[0]);
    if (!id) return;
    map[id] = { displayName: val_(r[1]), fullName: val_(r[2]), email: val_(r[3]), phone: val_(r[4]), instagram: val_(r[5]), skill: val_(r[6]), paymentHandle: val_(r[7]), photo: val_(r[8]), status: val_(r[9]) };
  });
  return map;
}
...
function getMatchesForSite_(ss) {
  ...
  if (!sh) return [];
  const values = sh.getDataRange().getValues();
  return values.slice(1).filter(function(r) { return r[1]; }).map(function(r, idx) {
    return { row: idx + 2, eventId: val_(r[0]), matchId: val_(r[1]), type: val_(r[2]), table: val_(r[3]), playerAId: val_(r[4]), playerA: val_(r[5]), playerBId: val_(r[6]), playerB: val_(r[7]), scoreA: Number(r[8]) || 0, scoreB: Number(r[9]) || 0, winnerId: val_(r[10]), winner: val_(r[11]), verified: r[12] === true || String(r[12]).toUpperCase() === "TRUE", status: val_(r[13]) || (r[12] === true ? "Final" : "Scheduled"), score: String((r[8] || 0) + "-" + (r[9] || 0)) };
  }).reverse();
}

function getWeeklyResults_(ss) {
  const payout = getPayout_(ss);
  return [{ week: payout.week, winner: payout.firstPlaceName || "TBD", players: payout.paidPlayers, collected: payout.totalCollected, organizerCut: payout.operationsCut, prizePool: payout.prizePool, first: payout.firstPlacePayout, second: payout.secondPlacePayout, third: payout.thirdPlacePayout }];
}
