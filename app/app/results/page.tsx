...
export default async function ResultsPage() {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-7xl px-5 py-10 text-white">
      ...
      <div className="mt-4 space-y-3">{matches.map((m, i) => (<Card key={i}><div className="flex flex-wrap items-center justify-between gap-4"><p className="font-black uppercase">{m.playerA} vs {m.playerB}</p><p className="text-white/60">{m.score}</p><p className="font-black text-rtt-red">{m.winner || m.status}</p></div></Card>))}</div>
    </main>
  );
}
