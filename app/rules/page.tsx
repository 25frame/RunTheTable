...
export default function RulesPage() {
  return (
    <main className="mx-auto min-h-screen max-w-4xl px-5 py-10 text-white">
      <p className="text-xs font-black uppercase tracking-[0.3em] text-rtt-red">RTT House Rules</p><h1 className="mt-3 text-6xl font-black italic uppercase">Rules</h1>
      <Card className="mt-8"><ol className="space-y-4">{rules.map((rule, index) => (<li key={rule} className="flex gap-4"><span className="font-black text-rtt-red">{String(index + 1).padStart(2, "0")}</span><span className="text-white/75">{rule}</span></li>))}</ol></Card>
    </main>
  );
}
