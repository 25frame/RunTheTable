...
export default async function StandingsPage() {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-7xl px-5 py-8 text-white">
      ...
      <h1 className="mt-3 text-5xl font-black italic uppercase leading-none md:text-7xl">
        Standings
      </h1>
          ...
          <Card key={p.id} className="p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-white/50">{p.skill}</p>
              </div>

              <Link
                href={`/players/${p.id}`}
                className="rounded-full bg-rtt-red px-4 py-2 text-xs font-black uppercase tracking-[0.15em]"
              >
                View
              </Link>
            </div>
            ...
            <div className="mt-5 grid grid-cols-3 gap-3 text-center">
              ...
              <div className="rounded-2xl bg-white/5 p-3">
                ...
                <p className="mt-1 text-xl font-black">{p.points}</p>
              </div>
      ...
      <div className="mt-8 hidden overflow-hidden rounded-[2rem] border border-white/10 bg-black/45 md:block">
        ...
        {sorted.map((p) => (
          <div
            key={p.id}
            className="grid grid-cols-6 items-center border-t border-white/10 px-4 py-4 text-sm odd:bg-white/[0.03]"
          >
            ...
            <span>{p.points}</span>
            <span>{p.pointDiff}</span>
            <Link
              className="font-bold text-rtt-red underline"
              href={`/players/${p.id}`}
            >
              View
            </Link>
