/**
 * Public league data API reader.
 *
 * Public pages only READ data from Apps Script.
 * Admin write actions live in lib/adminApi.ts.
 */
...
const FALLBACK_FORM_URL = process.env.NEXT_PUBLIC_GOOGLE_FORM_URL || "https://docs.google.com/forms/d/e/1FAIpQLScGDbgA5YOItre1EjvQIxlvi3pIByBDq10HFW24MAjOw7tZZA/viewform";

export async function getRTTData(): Promise<RTTData> {
  const res = await fetch(API_URL, { cache: "no-store" });

  if (!res.ok) {
    return { players: [], matches: [], weeklyResults: [], formUrl: FALLBACK_FORM_URL };
  }
